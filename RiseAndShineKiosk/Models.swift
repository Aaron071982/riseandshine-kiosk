...
enum CheckDirection: String, Codable, Hashable {
    ...
    var apiEventType: String { self == .in ? "IN" : "OUT" }
}
...
struct CreateAttendanceEventRequest: Codable {
    ...
    var signedByRelationship: String
    var signatureHash: String
    var signaturePngBase64: String
    var capturedAt: String?

    enum CodingKeys: String, CodingKey {
        case serviceClientId
        case scheduleAssignmentId
        case eventType
        case signedByName
        case signedByRelationship
        case signatureHash
        case signaturePngBase64
        case capturedAt
    }

    func encode(to encoder: Encoder) throws {
        var c = encoder.container(keyedBy: CodingKeys.self)
        try c.encode(serviceClientId, forKey: .serviceClientId)
        try c.encode(scheduleAssignmentId, forKey: .scheduleAssignmentId)
        try c.encode(eventType, forKey: .eventType)
        try c.encode(signedByName, forKey: .signedByName)
        try c.encode(signedByRelationship, forKey: .signedByRelationship)
        try c.encode(signatureHash, forKey: .signatureHash)
        try c.encode(signaturePngBase64, forKey: .signaturePngBase64)
